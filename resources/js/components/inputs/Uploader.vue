...
<script>
import VueSplash from "../layout/VueSplash.vue";
    ...
    init(){
      ...
      this.savedMedia.forEach((image, index) => {
        if(!this.savedMedia[index].url){
          this.savedMedia[index].url = image.name
        }
      });
      ...
      for(var i=0; i < files.length; i++){
        if(!this.max || this.allMedia.length < this.max){
          if(files[i].size <= this.maxFilesize*1000000){
            ...
            let addedImage = {url:url, name:files[i].name, file:files[i]}
            ...
            if(this.warnings){
              CustomNotification.swalMessage(
                  'error',
                  'Файл для загрузки превышает максимально допустимый размер: '+ this.maxFilesize +'MB'
              )
            }
            break;
          }
          ...
          if(this.warnings){
            CustomNotification.swalMessage(
                'error',
                'Было достигнуто макимальное количество файлов: '+ this.max
            )
          }
          break;
        }
